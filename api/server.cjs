...
async function analyzeVisionImage(referenceImage, userTopic, reqOrKey) {
        ...
        const response = await ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                subjectName: { type: import_genai.Type.STRING },
                productCategory: { type: import_genai.Type.STRING },
                detectedColors: {
                  type: import_genai.Type.ARRAY,
                  items: { type: import_genai.Type.STRING }
                },
                patternsAndMotifs: { type: import_genai.Type.STRING },
                materialTexture: { type: import_genai.Type.STRING },
                distinctiveDetails: {
                  type: import_genai.Type.ARRAY,
                  items: { type: import_genai.Type.STRING }
                },
                targetVibe: { type: import_genai.Type.STRING },
                fabricDetails: { type: import_genai.Type.STRING },
                culturalSignificance: { type: import_genai.Type.STRING },
                luxuryLevel: { type: import_genai.Type.STRING },
                disclaimer: { type: import_genai.Type.STRING }
              },
...
function generateSmartFallbackCampaign(topic, tone = "professional", imageStyle = "photorealistic", selectedPlatforms = ["linkedin", "twitter", "instagram"], hasReferenceImage = false, visionAnalysis = null, businessProfile = null, outputFormatMode = "editorial_showcase") {
  ...
  const imagePromptStyleSuffix = outputFormatMode === "marketing_poster" ? `Designed as an ultra-high-converting promotional marketing poster for ${bizName}, framing ${coreSubject} (${colorsStr}) with elegant studio lighting, clean graphic border layout, professional brand typography display, photorealistic 8k studio render` : `An ultra-high-end fashion editorial showcase photo of a model displaying ${coreSubject} (${colorsStr}), exquisite studio ambient lighting, rich fabric texture, crisp 8k photorealistic magazine cover shot`;
  const campaign = {
...
What details do you value most in handcrafted luxury? Share your thoughts below! \u{1F447}`,
  ...
  if (selectedPlatforms.includes("twitter")) {
    campaign.twitter = {
      tweet: `Why ${coreSubject} from ${bizName} is capturing everyone's attention right now \u{1F9F5}\u{1F447}

${bizHook}`,
      thread: [
        `Why ${coreSubject} from ${bizName} is capturing everyone's attention right now \u{1F9F5}\u{1F447}
...
${bizHook}`,
      ...
      abHookOptions: [
        `Why ${coreSubject} from ${bizName} is capturing everyone's attention right now \u{1F9F5}\u{1F447}`,
        `The secret behind authentic ${coreSubject} (${colorsStr}) \u{1F9F5}\u{1F447}`
      ],
      postingSlot: defaultPostingSlot,
      hashtagTiers: {
...
Which feature is your favorite? Double tap and drop your comments below! \u{1F4AC}\u2764\uFE0F`,
      ...
      abHookOptions: [
        ...
        `Swipe to feel the luxury: ${coreSubject} in ${colorsStr} \u{1F496}`
      ],
      postingSlot: defaultPostingSlot,
      hashtagTiers: {
...
Explore the collection today and find your perfect match! \u{1F447}`,
...
[0:45 - 0:60] Subscribe to ${bizName} for more exclusive heritage fashion highlights!`,
      ...
      abHookOptions: [
        ...
        `Unboxing the Most Luxurious ${coreSubject}! \u{1F31F}`
      ],
      postingSlot: defaultPostingSlot,
      hashtagTiers: {
  ...
  if (selectedPlatforms.includes("threads")) {
    campaign.threads = {
      threadOpener: `There's something irreplaceable about the authentic charm of ${coreSubject} from ${bizName}. What's your go-to style for festive occasions? \u{1F9F5}`,
      replyFollowUps: [
...
app.post("/api/generate-content", async (req, res) => {
  ...
  try {
    ...
    if (!ai) {
      ...
      const minDeepThinkingTimeMs2 = 3e3;
      const elapsedTime2 = Date.now() - startTime;
      if (elapsedTime2 < minDeepThinkingTimeMs2) {
        await new Promise((r) => setTimeout(r, minDeepThinkingTimeMs2 - elapsedTime2));
      }
...
Generate a valid JSON object matching the requested platforms.`;
    ...
    if (elapsedTime < minDeepThinkingTimeMs) {
      await new Promise((res2) => setTimeout(res2, minDeepThinkingTimeMs - elapsedTime));
    }
    if (!responseText) {
    ...
    try {
      ...
      const merged = {
        ...
        ...parsedData,
        selectedPlatforms,
        referenceImage: hasReferenceImage ? referenceImage : void 0,
...
- "changeSummary": a brief 1-sentence description of what was updated.`;
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              updatedContent: { type: import_genai.Type.STRING },
              changeSummary: { type: import_genai.Type.STRING }
            },
    ...
    if (action === "shorter") {
      ...
      text = currentContent + "\n\n\u{1F4A1} Key Insight: Building momentum consistently outperforms sporadic bursts of effort. Prioritize sustainable systems!";
      summary = "Expanded post with additional strategic insights.";
    } else if (action === "emojis") {
      text = "\u{1F680} " + currentContent.replace(/\. /g, ". \u2728 ");
...
app.post("/api/social/publish", async (req, res) => {
  try {
    ...
    const ayrshareApiKey = process.env.AYRSHARE_API_KEY || req.headers["x-ayrshare-api-key"];
    if (ayrshareApiKey) {
...
app.get("/api/auth/meta/login-url", (req, res) => {
  const appId = req.query.appId || process.env.META_APP_ID || "YOUR_FREE_META_APP_ID";
...
app.get("/api/auth/linkedin/login-url", (req, res) => {
  const clientId = req.query.clientId || process.env.LINKEDIN_CLIENT_ID || "YOUR_FREE_LINKEDIN_CLIENT_ID";
...
var LocalVectorDB = class {
  ...
  querySimilarContext(queryText, topK = 3) {
...
var vectorDB = new LocalVectorDB();
function tokenSummarizerMiddleware(req, res, next) {
  if (req.body) {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              hookTitle: { type: import_genai.Type.STRING },
              targetVibe: { type: import_genai.Type.STRING },
              storyboard: {
                type: import_genai.Type.ARRAY,
                items: {
                  ...
                  properties: {
                    ...
                    visualShot: { type: import_genai.Type.STRING },
                    voiceoverScript: { type: import_genai.Type.STRING },
                    textOnScreen: { type: import_genai.Type.STRING }
                  },
                  required: ["timestamp", "visualShot", "voiceoverScript", "textOnScreen"]
                }
              },
              recommendedAudioHook: { type: import_genai.Type.STRING }
            },
      ...
      storyboard: [
        {
          ...
          textOnScreen: "\u{1F525} WATCH THIS FIRST!"
        },
        {
          timestamp: "3s - 10s",
        ...
        },
        {
          ...
          textOnScreen: "\u{1F449} Tap Link in Bio to Order!"
        }
      ],
...
Original Content: "${content}"`;
    ...
    if (targetLanguage === "marathi") {
      fallback = `\u{1F338} \u0909\u0924\u094D\u0915\u0943\u0937\u094D\u091F \u0917\u0941\u0923\u0935\u0924\u094D\u0924\u093E \u0906\u0923\u093F \u0905\u0938\u094D\u0938\u0932 \u092A\u0930\u0902\u092A\u0930\u093E! ${content} 

\u{1F449} \u0906\u091C\u091A \u0911\u0930\u094D\u0921\u0930 \u0915\u0930\u093E \u0915\u093F\u0902\u0935\u093E \u0938\u0902\u092A\u0930\u094D\u0915 \u0938\u093E\u0927\u093E!`;
    } else if (targetLanguage === "hindi") {
      fallback = `\u2728 \u0936\u093E\u0939\u0940 \u0905\u0902\u0926\u093E\u091C\u093C \u0914\u0930 \u092A\u094D\u0930\u0940\u092E\u093F\u092F\u092E \u0915\u094D\u0935\u093E\u0932\u093F\u091F\u0940! ${content} 

\u{1F449} \u0905\u092D\u0940 \u0911\u0930\u094D\u0921\u0930 \u0915\u0930\u0947\u0902!`;
    } else if (targetLanguage === "hinglish") {
      fallback = `\u{1F525} High quality & premium vibe! ${content} 

\u{1F449} DM us now to order!`;
    }
...
Platform: ${platform || "instagram"}.`;
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              nicheHashtags: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              trendingHashtags: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              recommendedAudioHooks: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              estimatedViralityBoost: { type: import_genai.Type.STRING }
            },
    ...
    res.json({
      ...
      recommendedAudioHooks: [
        "\u{1F3B5} 'Aesthetic Ethnic Vibe' by Instrumental Chill",
        "\u{1F3B5} 'Upbeat Royalty Beat' (Viral Reels Audio)",
        "\u{1F3B5} 'Subtle Luxury Experience' Sound"
      ],
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`\u{1F680} Social Content Generator server running on http://0.0.0.0:${PORT}`);
  });
}
startServer();
